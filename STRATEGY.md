...
- 台灣市場波動較大、流動性較低，可能需要調整 N/M 窗口或門檻以適應在地市場特性
- **策略邏輯跨市場有效**，但最佳參數不具普適性 — 印證了 CSCV 的警告
...
當前策略在 signal < -0.8 時清倉持有現金。可考慮以下延伸：

| 方案 | 操作 | 適用場景 | 風險 |
|------|------|----------|------|
| **反向做空** | signal < -0.8 時 Short SPY/台指期 | 明確空頭趨勢 | 軌空風險大，需止損 |
| **VIX 避險** | 空倉時買入 VIX Call/UVXY | 黑天魚事件對沖 | 持有成本高（VIX 衰減） |
| **債券輪動** | 空倉時轉持 TLT/公債 | 始終有收益 | 利率風險 |
| **降榿台指期** | 信號較弱時只持小台期 | 漸進式風控 | 實作複雜度高 |

> 建議最具商業價值的方案是「債券輪動」：策略空倉時轉持小台期或公債 ETF，提升資金使用效率，同時避免裸空風險。
